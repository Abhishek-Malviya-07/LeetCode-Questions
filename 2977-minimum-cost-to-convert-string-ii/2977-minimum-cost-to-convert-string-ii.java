class Solution {
    public long minimumCost(String source, String target, String[] original, String[] changed, int[] cost) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int len : lengths) {
                if (i - len >= 0) {
                    String subSrc = source.substring(i - len, i);
                    String subTgt = target.substring(i - len, i);
                    
                    if (stringToId.containsKey(subSrc) && stringToId.containsKey(subTgt)) {
                        int u = stringToId.get(subSrc);
                        int v = stringToId.get(subTgt);
